...
使用 Nginx 做代理服务器，监听的端口是 8000，而后端服务器的端口是 80，一般的请求都能被正常处理，但是当服务器端返回重定向 (302) 时，客户端浏览器的 url 端口就丢了，页面也就无法访问了。经过调查 [Nginx 文档](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_redirect), 以下配置解决了此问题。
